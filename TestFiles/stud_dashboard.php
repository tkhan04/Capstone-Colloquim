...
try {
    ...
    if ($student) {
        // ── Minimum events required (max across all active enrolled courses) ──
        $stmt = $pdo->prepare(
        ...
        // ── Count fully completed attendances ─────────────────────────────────
        // A record counts only when:
        //   • end_scan_time is present, AND EITHER
        //   • it was a professor/admin override (overridden_by IS NOT NULL), OR
        //   • the student checked in within 10 min of event start AND
        //     checked out no earlier than the event's scheduled end time.
        $stmt = $pdo->prepare(
            "SELECT COUNT(DISTINCT a.event_id)
             ...
             WHERE a.student_id = ?
               ...
               AND (
                   ...
                   OR (
                       a.start_scan_time <= DATE_ADD(ev.start_time, INTERVAL 10 MINUTE)
...
<body class="dashboard-page">
    ...
    <nav class="dashboard-nav">
        ...
        <div class="nav-user">
            <?php if ($student): ?>
            <!-- Show full name (friendly) with ID in tooltip -->
            <span title="ID: <?= $studentId ?>">
                Welcome, <?= htmlspecialchars($student['fname'] . ' ' . $student['lname']) ?>!
            </span>
            <?php endif; ?>
            <a href="index.html" class="btn-logout" title="Sign out"><i class="fas fa-sign-out-alt"></i></a>
        </div>
    </nav>
