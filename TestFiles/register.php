...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!preg_match('/^\d{7}$/', $userId)) {
    ...
    } elseif (!in_array($role, ['student', 'professor', 'admin'], true)) {
        ...
        try {
            ...
            // Admin role: only AppUser row is needed (no separate Admin table in schema)

            // PRG redirect with success message so refreshing doesn't re-POST
            header('Location: index.html?message=' . urlencode('Account created! Please sign in.'));
            exit;
...
<head>
    ...
    <style>
        ...
        .back-button:hover { background: #ff6600; }
    </style>
</head>
<body>
        ...
        <form method="POST" id="registerForm">
            ...
            <div class="input-group">
                ...
                <input type="email" name="email" id="emailInput"
                       placeholder="yourname@gettysburg.edu" required
                       value="<?= htmlspecialchars($_POST['email'] ?? '') ?>">
                <span id="email-error" style="color:#dc3545;font-size:.85rem;display:none;">
                    Email must end with @gettysburg.edu
                </span>
            </div>
            ...
            <!-- Year shown only when student role is selected -->
    ...
    // Restore year visibility on back-navigation / validation error
    toggleYear(document.getElementById('roleSelect').value);

    // Client-side Gettysburg email domain check (server also validates)
    document.getElementById('emailInput').addEventListener('blur', function () {
        const errEl = document.getElementById('email-error');
        if (this.value && !this.value.toLowerCase().endsWith('@gettysburg.edu')) {
